...
def test_creates_django_user():
    call_command("create_hr_user", email="hr@example.com", password="securepass123",
                 first_name="Jane", last_name="Doe")
    assert User.objects.filter(username="hr@example.com").exists()


@pytest.mark.django_db
def test_creates_employee_record():
    call_command("create_hr_user", email="hr@example.com", password="securepass123",
                 first_name="Jane", last_name="Doe")
    assert Employee.objects.filter(email="hr@example.com").exists()
...
def test_created_user_can_authenticate():
    ...
    assert authenticate(username="hr@example.com", password="securepass123") is not None


@pytest.mark.django_db
def test_duplicate_email_prints_error_and_does_not_raise():
    call_command("create_hr_user", email="hr@example.com", password="pass123",
                 first_name="Jane", last_name="Doe")
    err = StringIO()
    call_command("create_hr_user", email="hr@example.com", password="pass456",
                 first_name="Jane", last_name="Doe", stderr=err)
